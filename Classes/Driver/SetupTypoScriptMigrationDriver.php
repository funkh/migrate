...
class SetupTypoScriptMigrationDriver extends AbstractSystemTemplateMigrationDriver {
	...
	public function migrate($package) {
		...
		$migrationsPath = $this->getAbsoluteMigrationScriptPathByType(MigrationDriverInterface::TYPE_TYPOSCRIPT_TEMPLATE_SETUP);
		if (is_dir($migrationsPath) && isset($this->configuration['TypoScript']['Template']['Setup'])) {
			foreach ($this->configuration['TypoScript']['Template']['Setup'] as $migrationFileName => $configuration) {
				...
				if (
					...
					|| $this->hasMigration(MigrationDriverInterface::TYPE_TYPOSCRIPT_TEMPLATE_SETUP, $this->getPackageVersion(), $migrationFileName)
				) {
					continue;
				}
				...
				if (
					...
					$this->addMigration(
						MigrationDriverInterface::TYPE_TYPOSCRIPT_TEMPLATE_SETUP,
