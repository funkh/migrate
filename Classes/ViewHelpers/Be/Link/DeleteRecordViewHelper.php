 ...
 *
 */
class DeleteRecordViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractTagBasedViewHelper {
	...
	public function render(\TYPO3\CMS\Extbase\DomainObject\DomainObjectInterface $object, $backPath = '', $returnUrl = '', $showSpriteIcon = TRUE) {
		$aOnClick = 'return deleteRecord(\'' . $this->resolveTableName($object) . '\',\'' . (int)$object->getUid() . '\', unescape(\'' . rawurlencode($returnUrl) . '\'));';

		$this->tag->addAttribute('href', '#');
		$this->tag->addAttribute('onclick', $aOnClick);
		if ($showSpriteIcon === TRUE) {
			$this->tag->setContent(IconUtility::getSpriteIcon('actions-edit-delete'));
